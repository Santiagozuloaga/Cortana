...
def _init_rich():
    global _RICH, console, Console, Markdown, Live, Syntax, Panel, rprint
    if _RICH is None:
        try:
            ...
            Console, Markdown, Live, Syntax, Panel, rprint = _Console, _Markdown, _Live, _Syntax, _Panel, _rprint
            _RICH = True
            console = Console()
        except ImportError:
            _RICH = False
...
def _make_renderable(text: str):
    ...
    if _init_rich() and any(c in text for c in ("#", "*", "`", "_", "[")):
...
def _start_live() -> None:
    ...
    if _init_rich() and _RICH_LIVE and _current_live is None:
...
Choose experts whose domains are most relevant to analyzing "{topic}" from different angles."""
    ...
    try:
        ...
        # Corrección Bug #1: loggear en lugar de silenciar — ayuda a depurar errores de API
        warn(f"_generate_personas falló ({type(e).__name__}: {e})")
        return None
...
def _tg_poll_loop(token: str, chat_id: int, config: dict):
    ...
    while not _telegram_stop.is_set():
        try:
            ...
            # Corrección Bug #1: loggear error del polling loop — sin esto el bot
            # falla silenciosamente y el usuario no sabe por qué dejó de responder
            traceback.print_exc()
            print(f"\n[telegram poll error]: {type(e).__name__}: {e}", flush=True)
            _telegram_stop.wait(5)
...
def repl(config: dict, initial_prompt: str = None):
    ...
    _is_dumb = (_init_rich() and getattr(console, "is_dumb_terminal", False))
...
def main():
    ...
    parser.add_argument("--no-tools", action="store_true",
                        help="Disable tool use (required for Gemma and other models without function calling)")
