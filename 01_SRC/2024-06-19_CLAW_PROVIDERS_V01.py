...
# ── Corrección Bug #3/#7/#9: Thinking seguro ──────────────────────────────

# Modelos Claude que soportan extended thinking (verificado en API)
...
def _model_supports_thinking(model: str) -> bool:
    ...
    # Verificar override por variable de entorno (igual que thinking.py)
    for tier_model_env, tier_caps_env in [
    ...
    # Modelos de terceros conocidos sin thinking
    for prefix in ("qwen", "deepseek", "llama", "mistral", "gemma", "phi", "grok"):
        if prefix in model_lower:
            logger.debug(f"Thinking desactivado para '{model}' (modelo de tercero)")
            return False

    # Lista conocida de modelos Claude con thinking
    return any(name in model_lower for name in _CLAUDE_THINKING_MODELS)


def _safe_thinking_budget(raw) -> int:
    """
    Corrección Bug #9: Valida budget_tokens (evita NaN del parseInt original).
    """
    try:
        val = int(raw)
        if val <= 0:
            logger.warning(f"thinking_budget inválido ({raw!r}), usando 10000")
            return 10000
        return min(val, 100_000)
    except (ValueError, TypeError):
        logger.warning(f"thinking_budget no es número ({raw!r}), usando 10000")
        return 10000
...
def stream_anthropic(
    ...
    # Corrección Bug #3/#9: verificar modelo ANTES de enviar thinking
    if config.get("thinking") and _model_supports_thinking(model):
        budget = _safe_thinking_budget(config.get("thinking_budget", 10000))
        kwargs["thinking"] = {
            "type":          "enabled",
            "budget_tokens": budget,
        }
    elif config.get("thinking") and not _model_supports_thinking(model):
        logger.info(
            f"Thinking solicitado pero '{model}' no lo soporta — omitido. "
            "Usa /thinking solo con modelos Claude claude-sonnet-4/opus-4."
        )
...
def stream_ollama(
    ...
    model_base = model.split("/")[-1].split(":")[0].lower()
    model_supports_tools = any(
        model_base.startswith(m) for m in MODELS_WITH_TOOL_SUPPORT
    )

    # Qwen3 tiene modo thinking que manda content vacio — desactivarlo
    ...
    yield AssistantTurn(full_text, tool_calls_list, 0, 0)
